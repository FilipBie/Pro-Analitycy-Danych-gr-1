...
```{r miesieczna migawka ofert wynajmu, echo=FALSE, include=TRUE}
...
```{r tabela podsumowująca liczby i proporcje wartosci NA, echo=FALSE, include=TRUE}
...
```{r Wizualizacja brakow danych w calym zbiorze, echo=FALSE, include=TRUE}

vis_miss(data) +
  ggtitle("Wizualizacja brakow danych w calym zbiorze")
```

```{r Wizualizacja brakow danych w poszczegolnych kolumnacjh, echo=FALSE, include=TRUE}
gg_miss_var(data) +
  ggtitle("Wizualizacja brakow danych w poszczegolnych kolumnach")
```

```{r Mapa brakow danych, echo=FALSE, include=TRUE}
...
```{r mieszkania_z_winda, echo=FALSE, include=TRUE}
...
stats_long <- stats %>% pivot_longer(cols = everything(),
                                     names_to=c("Variable", "Statistic"),
                                     names_sep = "_"
                                     ) %>% pivot_wider(names_from=Variable, values_from=value)

datatable(stats_long, options=list(pageLength=8,autoWidth=TRUE),caption="Podstawowe statystyki opisowe dla kluczowych zmiennych")
...
```{r Szacowanie korelacji dla wybranych danych ilościowych, echo=FALSE, include=FALSE}
...
```{r Macierzy oszacowanych korelacji, echo=FALSE, include=TRUE}
library(corrplot)
